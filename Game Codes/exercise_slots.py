# Found at https://python-forum.io/thread-6149.html
...
def game_loop():
    global reel_1, reel_2, reel_3, bank, current_bet, total_lost, total_won
    if bank <= 0:
        print("GAME OVER!")
    while bank > 0:
        if current_bet > 0 and bank >= current_bet:
            bet_r = input("\n[ENTER] TO REPEAT BET, 'EXIT' TO LEAVE, 'NO' TO PLACE NEW ONE: ")
            if bet_r == "" or bet_r == "Y":
                bet = current_bet
            elif bet_r.lower() == "exit":
                exit()
            else:
                current_bet = 0
                bet = 0
...
def start():
    ...
    command = input("PRESS [ENTER] TO BEGIN OR CANCEL TO LEAVE")
    if command == "":
        bank = 10000
        game_loop()
